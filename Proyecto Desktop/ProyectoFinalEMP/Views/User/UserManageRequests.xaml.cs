...
namespace ProyectoFinalEMP.Views.User
{
    ...
    public partial class UserManageRequests : Page
    {
        private bool barraExpandida = false;
        private List<BsonDocument> solicitudesFiltradas = new();


        public UserManageRequests(string tipoSolicitud)
        {
            InitializeComponent();
        ...
        private async void CargarSolicitudes(string estado)
        {
            ...
            var solicitudes = await GlobalData.Instance.miBBDD.ObtenerSolicitudesUsuario(usuarioId, estado);
            solicitudesFiltradas = solicitudes;

            var datosTabla = solicitudes.Select(solicitud => new FilaSolicitud
            {
                Documento = solicitud,
