...
namespace ProyectoFinalEMP.Views.User
{
    ...
    public partial class UserMainView : Page
    {
        ...
        {
            if (ComboBoxServiciosTecnicos.SelectedItem is ComboBoxItem selectedItem)
            {
        ...
        #region Boton continuar tarjeta de la derecha
        private void ContinuarAsesoria_Click(object sender, RoutedEventArgs e)
        {
            if (ComboBoxAsesoriaAdmin.SelectedItem is ComboBoxItem selectedItem)
            {
                string ruta = selectedItem.Tag as string;
                if (!string.IsNullOrEmpty(ruta))
                {
